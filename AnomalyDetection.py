...
def webpage_downloaded_file_checking():
    subprocess.Popen("adb -s "+device_id+" pull $(adb -s "+device_id+" shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/Webpage.xml",
                     shell=True)
    ...
    if verify_file_existence_output_01 != '':
        ...
        if verify_file_existence_output_02 != '':
            ...
            adb_verify_webview_download_command = r'''adb -s '''+device_id+''' logcat -d MediaProvider:D *:S | grep /storage/emulated/0/Download/'''
...
def android_webview_crash_checking():
    adb_verify_webview_crash_command = r'''adb -s '''+device_id+''' logcat -d ActivityManager:I *:S | grep "Scheduling restart of crashed service" | grep org.chromium.content.app.SandboxedProcessService'''
...
def android_webview_anomaly_checking(verification_text):
    ...
    if crash:
        ...
        if download == 1 or download == 3:
            ...
            subprocess.Popen(r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/Webview.xml''',
                             shell=True)
...
def chrome_custom_tab_crash_checking():
    subprocess.run(r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/ChromeCustomTab.xml''',
                   shell=True)
...
def line(website, verification_text):
    ...
    check_line_installation = subprocess.Popen("adb -s "+device_id+" shell pm list packages | grep jp.naver.line.android", shell=True,
                                               stdout=subprocess.PIPE)
    ...
    if check_line_installation_output == '':
        ...
        subprocess.run("adb -s "+device_id+" install ~/Desktop/APKs/line*.apk", shell=True)
        print("LINE installation finished")

    # Check if login is required
    subprocess.run("adb -s "+device_id+" logcat -c", shell=True)
    subprocess.run("adb -s "+device_id+" shell am start -n jp.naver.line.android/.activity.SplashActivity", shell=True)
    time.sleep(10)
    check_line_registration_required = subprocess.Popen(
        "adb -s "+device_id+" logcat -d ActivityTaskManager:I *:S | grep jp.naver.line.android/com.linecorp.registration.ui.RegistrationActivity",
    ...
    if registration_required:
        ...
        # Enter Chats
        os.system("adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 66")
        time.sleep(3)
    ...
    os.system("adb -s "+device_id+" shell input text $url")
    time.sleep(5)
    os.system("adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 66")
    time.sleep(3)
    ...
    for x in range(3):
        ...
        while not file_pulled:
            pull_line_chat_xml = subprocess.Popen(
                r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/KeepMemo.xml''', shell=True,
                stdout=subprocess.PIPE)
            ...
            if pull_line_chat_xml_output[:23] == "/sdcard/window_dump.xml":
                ...
                file_pulled = False
        adb_open_url_in_webview_command = r'''url=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /text="''' + formatted_line_url + '''"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/KeepMemo.xml) ; adb -s '''+device_id+''' shell input tap $url'''
        ...
        if result != 1:
            ...
            adb_close_line_in_app_browser_command = r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/LineWebview.xml ; coords=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /resource-id="jp.naver.line.android:id\/iab_header_close"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/LineWebview.xml) ; adb -s '''+device_id+''' shell input tap $coords'''
...
def telegram(website, verification_text):
    ...
    check_telegram_installation = subprocess.Popen("adb -s "+device_id+" shell pm list packages | grep org.telegram.messenger",
                                                   shell=True, stdout=subprocess.PIPE)
    ...
    if check_telegram_installation_output == '':
        ...
        subprocess.run("adb -s "+device_id+" install ~/Desktop/APKs/telegram*.apk", shell=True)
        print("Telegram installation finished")

    # Check if login is required
    subprocess.run("adb -s "+device_id+" logcat -c", shell=True)
    subprocess.run("adb -s "+device_id+" shell am start -n org.telegram.messenger/org.telegram.ui.LaunchActivity", shell=True)
    time.sleep(15)
    check_telegram_registration_required = subprocess.Popen(
        "adb -s "+device_id+" logcat -d ActivityTaskManager:I *:S | grep org.telegram.messenger/org.telegram.ui.IntroActivity | grep Displayed",
        shell=True, stdout=subprocess.PIPE)
    ...
    adb_open_telegram_navigation_menu_command = r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/ChatsPage.xml ; navMenu=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /content-desc="Open navigation menu"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/ChatsPage.xml) ; adb -s '''+device_id+''' shell input tap $navMenu'''
    subprocess.Popen(adb_open_telegram_navigation_menu_command, shell=True)
    time.sleep(5)
    adb_open_telegram_saved_messages_command = r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/NavMenu.xml ; savedMessages=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /text="Saved Messages"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/NavMenu.xml) ; adb -s '''+device_id+''' shell input tap $savedMessages'''
    ...
    for x in range(3):
        ...
        while not file_pulled:
            pull_telegram_chat_xml = subprocess.Popen(
                r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/Chat.xml''', shell=True,
                stdout=subprocess.PIPE)
            ...
            if pull_telegram_chat_xml_output[:23] == "/sdcard/window_dump.xml":
                ...
                file_pulled = False
        adb_open_url_in_webview_command = r'''url=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /text="''' + formatted_telegram_url + '''"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/Chat.xml) ; adb -s '''+device_id+''' shell input tap $url'''
        subprocess.Popen(adb_open_url_in_webview_command, shell=True)
        time_countdown = 45.0
        while time_countdown > 0:
            start = time.time()
        ...
        if result != 1:
            if result == 3:
                verify_chrome_first_activity = subprocess.Popen(
                    "adb -s "+device_id+" logcat -d ActivityTaskManager:I *:S | grep Displayed | grep com.android.chrome/org.chromium.chrome.browser.firstrun.FirstRunActivity",
                    shell=True, stdout=subprocess.PIPE)
                ...
                if verify_chrome_first_activity_output != '':
                    ...
                    os.system(
                        "adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 66")
                    time.sleep(3)
                    os.system("adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 66")
                    time.sleep(5)
                else:
        ...
        if result == 5:
            subprocess.Popen("adb -s "+device_id+" shell input keyevent 4", shell=True)
            time.sleep(3)
        ...
        adb_close_web_view_command = "adb -s "+device_id+" shell input keyevent 4"
...
def facebookmessenger(website, verification_text):
    ...
    check_facebook_messenger_installation = subprocess.Popen("adb -s "+device_id+" shell pm list packages | grep com.facebook.orca",
                                                             shell=True, stdout=subprocess.PIPE)
    ...
    if check_facebook_messenger_installation_output == '':
        ...
        subprocess.run("adb -s "+device_id+" install ~/Desktop/APKs/messenger*.apk", shell=True)
        print("Facebook Messenger installation finished")

    subprocess.run("adb -s "+device_id+" shell svc wifi disable", shell=True)
    time.sleep(3)
    subprocess.run("adb -s "+device_id+" logcat -c", shell=True)
    subprocess.run("adb -s "+device_id+" shell am start -n com.facebook.orca/.auth.StartScreenActivity", shell=True)
    time.sleep(10)

    # Check if login is required
    check_facebook_messenger_registration_required = subprocess.Popen(
        "adb -s "+device_id+" logcat -d ActivityTaskManager:I *:S | grep com.facebook.orca/com.facebook.messaging.accountlogin.AccountLoginActivity",
        shell=True, stdout=subprocess.PIPE)
    ...
    if registration_required:
        ...
        os.system("adb -s "+device_id+" shell svc wifi enable")
        time.sleep(7)
    ...
    subprocess.Popen(
        r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/MessengerChats.xml ; newMessage=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /content-desc="New message"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/MessengerChats.xml) ; adb -s '''+device_id+''' shell input tap $newMessage''',
        shell=True)
    time.sleep(5)
    username = username.replace(" ", "%s")
    os.putenv("username", username)
    os.system("adb -s "+device_id+" shell input text $username")
    time.sleep(5)
    os.system("adb -s "+device_id+" shell svc wifi enable")
    time.sleep(10)
    os.system("adb -s "+device_id+" shell input keyevent 61 ; adb -s "+device_id+" shell input keyevent 66")
    ...
    subprocess.Popen(
        r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/SelfMessage.xml ; msg=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /text="Aa"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/SelfMessage.xml) ; adb -s '''+device_id+''' shell input tap $msg''',
        shell=True)
    time.sleep(3)
    os.putenv("url", website)
    os.system("adb -s "+device_id+" shell input text $url")
    ...
    for x in range(3):
        ...
        while not file_pulled:
            pull_messenger_chat_xml = subprocess.Popen(
                r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/OwnProfile.xml''', shell=True,
                stdout=subprocess.PIPE)
            ...
            if pull_messenger_chat_xml_output[:23] == "/sdcard/window_dump.xml":
                ...
                file_pulled = False
        adb_open_url_in_webview_command = r'''url=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /text="''' + formatted_messenger_url + '''"[^>]*content-desc=""[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/OwnProfile.xml) ; adb -s '''+device_id+''' shell input tap $url'''
        subprocess.Popen(adb_open_url_in_webview_command, shell=True)
        time_countdown = 45.0
        while time_countdown > 0:
            start = time.time()
        ...
        if result != 1:
            if result == 2:
                ...
                subprocess.Popen("adb -s "+device_id+" shell input keyevent 4 ; sleep 3 ; adb -s "+device_id+" shell input keyevent 4", shell=True)
                time.sleep(3)

            adb_close_web_view_command = r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/MessengerWebView.xml ; closeBrowser=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /content-desc="Close browser"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/MessengerWebView.xml) ; adb -s '''+device_id+''' shell input tap $closeBrowser'''
            subprocess.Popen(adb_close_web_view_command, shell=True)
...
for urls in url_file:
    ...
    subprocess.Popen("rm /tmp/*.xml", shell=True)
    time.sleep(3)
    os.system("adb -s "+device_id+" shell input keyevent KEYCODE_APP_SWITCH")
    time.sleep(3)
    subprocess.Popen(
        r'''adb -s '''+device_id+''' pull $(adb -s '''+device_id+''' shell uiautomator dump | grep -oP '[^ ]+.xml') /tmp/RecentApps.xml ; closeRecentApps=$(perl -ne 'printf "%d %d\n", ($1+$3)/2, ($2+$4)/2 if /text="Close all"[^>]*bounds="\[(\d+),(\d+)\]\[(\d+),(\d+)\]"/' /tmp/RecentApps.xml) ; adb -s '''+device_id+''' shell input tap $closeRecentApps''',
        shell=True)
    log_file.write("\n===================================================================================================\n\n")
    time.sleep(5)
